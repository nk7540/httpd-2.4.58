...
{
    ap_log_error(APLOG_MARK, APLOG_DEBUG, 0, ap_server_conf, APLOGNO(00263) "pid %" APR_PID_T_FMT " taking over scoreboard slot from "
                                                                            "%" APR_PID_T_FMT "%s",
                 newpid,
...
static void accept_mutex_error(const char *func, apr_status_t rv, int process_slot)
{
    ...
    else if (requests_this_child == INT_MAX || ((requests_this_child == ap_max_requests_per_child) && ap_max_requests_per_child))
    {
        ap_log_error(APLOG_MARK, level, rv, ap_server_conf, APLOGNO(00272) "apr_proc_mutex_%s failed "
                                                                           "before this child process served any requests.",
                     func);
        clean_child_exit(APEXIT_CHILDSICK);
    }
    ap_log_error(APLOG_MARK, level, rv, ap_server_conf, APLOGNO(00273) "apr_proc_mutex_%s failed. Attempting to "
...
static void *APR_THREAD_FUNC listener_thread(apr_thread_t *thd, void *dummy)
{
    ...
    while (1)
    {
        ...
        if ((rv = SAFE_ACCEPT(apr_proc_mutex_lock(my_bucket->mutex))) != APR_SUCCESS)
        {
        ...
        if (!listener_may_exit)
        {
            ...
            if ((rv = SAFE_ACCEPT(apr_proc_mutex_unlock(my_bucket->mutex))) != APR_SUCCESS)
            {
        ...
        else
        {
            if ((rv = SAFE_ACCEPT(apr_proc_mutex_unlock(my_bucket->mutex))) != APR_SUCCESS)
            {
                ...
                ap_log_error(APLOG_MARK, level, rv, ap_server_conf, APLOGNO(00274) "apr_proc_mutex_unlock failed. Attempting to "
                                                                                   "shutdown process gracefully.");
                signal_threads(ST_GRACEFUL);
            }
            break;
...
static void create_listener_thread(thread_starter *ts)
{
    ...
    if (rv != APR_SUCCESS)
    {
        ap_log_error(APLOG_MARK, APLOG_ALERT, rv, ap_server_conf, APLOGNO(00275) "ap_thread_create: unable to create listener thread");
...
static void setup_threads_runtime(void)
{
    ...
    if (rv != APR_SUCCESS)
    {
        ap_log_error(APLOG_MARK, APLOG_ALERT, rv, ap_server_conf, APLOGNO(03141) "ap_queue_info_create() failed");
        clean_child_exit(APEXIT_CHILDFATAL);
    }
    ...
    if (rv != APR_SUCCESS)
    {
        ap_log_error(APLOG_MARK, APLOG_EMERG, rv, ap_server_conf, APLOGNO(03285) "Couldn't create pollset in thread;"
    ...
    for (lr = my_bucket->listeners; lr != NULL; lr = lr->next)
    {
        ...
        if (rv != APR_SUCCESS)
        {
            ap_log_error(APLOG_MARK, APLOG_EMERG, rv, ap_server_conf, APLOGNO(03286) "Couldn't create add listener to pollset;"
                                                                                     " check system or user limits");
            /* let the parent decide how bad this really is */
            clean_child_exit(APEXIT_CHILDSICK);
        }
...
static void *APR_THREAD_FUNC start_threads(apr_thread_t *thd, void *dummy)
{
    ...
    while (1)
    {
        ...
        for (i = 0; i < threads_per_child; i++)
        {
            ...
            if (rv != APR_SUCCESS)
            {
                ap_log_error(APLOG_MARK, APLOG_ALERT, rv, ap_server_conf, APLOGNO(03142) "ap_thread_create: unable to create worker thread");
                /* let the parent decide how bad this really is */
                clean_child_exit(APEXIT_CHILDSICK);
            }
            threads_created++;
                         ...
                         int mode)
{
    ...
    for (i = 0; i < threads_per_child; i++)
    {
        ...
        { /* if we ever created this thread */
            ...
            if (rv != APR_SUCCESS)
            {
                ap_log_error(APLOG_MARK, APLOG_CRIT, rv, ap_server_conf, APLOGNO(00278) "apr_thread_join: unable to join worker "
                                                                                        "thread %d",
                             i);
            }
        }
...
static void join_start_thread(apr_thread_t *start_thread_id)
{
    ...
    if (rv != APR_SUCCESS)
    {
        ap_log_error(APLOG_MARK, APLOG_CRIT, rv, ap_server_conf, APLOGNO(00279) "apr_thread_join: unable to join the start "
                                                                                "thread");
    }
}
...
static void child_main(int child_num_arg, int child_bucket)
{
    ...
    if (rv != APR_SUCCESS)
    {
        ap_log_error(APLOG_MARK, APLOG_EMERG, rv, ap_server_conf, APLOGNO(00280) "Couldn't initialize cross-process lock in child");
        clean_child_exit(APEXIT_CHILDFATAL);
    }
    ...
    if (ap_thread_stacksize != 0)
    {
        ...
        {
            ap_log_error(APLOG_MARK, APLOG_WARNING, rv, ap_server_conf, APLOGNO(02435) "WARNING: ThreadStackSize of %" APR_SIZE_T_FMT " is "
    ...
    if (rv != APR_SUCCESS)
    {
        ap_log_error(APLOG_MARK, APLOG_ALERT, rv, ap_server_conf, APLOGNO(00282) "ap_thread_create: unable to create worker thread");
        /* let the parent decide how bad this really is */
        clean_child_exit(APEXIT_CHILDSICK);
    }
...
static int make_child(server_rec *s, int slot, int bucket)
{
    ...
    if ((pid = fork()) == -1)
    {
        ap_log_error(APLOG_MARK, APLOG_ERR, errno, s, APLOGNO(00283) "fork: Unable to fork new process");
    ...
    if (!pid)
    {
        ...
        if (status != OK)
            ap_log_error(APLOG_MARK, APLOG_DEBUG, errno,
                         ap_server_conf, APLOGNO(00284) "processor unbind failed");
#endif
...
static void perform_idle_server_maintenance(int child_bucket)
{
    ...
    for (i = 0; i < ap_daemons_limit; ++i)
    {
        ...
        if (any_dead_threads && totally_free_length < retained->idle_spawn_rate[child_bucket] && free_length < MAX_SPAWN_RATE / num_buckets && (!ps->pid /* no process in the slot */
                                                                                                                                                || ps->quiescing))
    ...
    if (retained->sick_child_detected)
    {
        ...
        else
        {
            ...
            ap_log_error(APLOG_MARK, APLOG_ALERT, 0,
                         ap_server_conf, APLOGNO(02325) "A resource shortage or other unrecoverable failure "
            ...
            if (active_thread_count >= max_workers / num_buckets)
            {
                ...
                if (0 == idle_thread_count)
                {
                    if (!retained->maxclients_reported)
                    {
                        ...
                        ap_log_error(APLOG_MARK, APLOG_ERR, 0, ap_server_conf, APLOGNO(00286) "server reached MaxRequestWorkers "
                ...
                else
                {
                    ...
                    {
                        ap_log_error(APLOG_MARK, APLOG_ERR, 0, ap_server_conf, APLOGNO(00287) "server is within MinSpareThreads of "
        ...
        else
        {
            ...
            if (retained->idle_spawn_rate[child_bucket] >= 8)
            {
                ap_log_error(APLOG_MARK, APLOG_INFO, 0,
                             ap_server_conf, APLOGNO(00289) "server seems busy, (you may need "
    ...
    while (!retained->mpm->restart_pending && !retained->mpm->shutdown_pending)
    {
        ...
        if (pid.pid != -1)
        {
            ...
            if (processed_status == APEXIT_CHILDFATAL)
            {
                ...
                {
                    ap_log_error(APLOG_MARK, APLOG_WARNING, 0, ap_server_conf, APLOGNO(00290) "Ignoring fatal error in child of previous "
                                                                                              "generation (pid %ld).",
                                 (long)pid.pid);
            ...
            if (child_slot >= 0)
            {
                ...
                else if (remaining_children_to_start && child_slot < ap_daemons_limit)
                {
            ...
            else if (ap_unregister_extra_mpm_process(pid.pid, &old_gen) == 1)
            {
                worker_note_child_killed(-1, /* already out of the scoreboard */
                                         pid.pid, old_gen);
                if (processed_status == APEXIT_CHILDSICK && old_gen == retained->mpm->my_generation)
                {
                    /* resource shortage, minimize the fork rate */
...
static int worker_run(apr_pool_t *_pconf, apr_pool_t *plog, server_rec *s)
{
    ...
    ap_log_error(APLOG_MARK, APLOG_NOTICE, 0, ap_server_conf, APLOGNO(00292) "%s configured -- resuming normal operations",
                 ap_get_server_description());
    ap_log_error(APLOG_MARK, APLOG_INFO, 0, ap_server_conf, APLOGNO(00293) "Server built: %s", ap_get_server_built());
    ap_log_command_line(plog, s);
    ap_log_mpm_common(s);
    ap_log_error(APLOG_MARK, APLOG_DEBUG, 0, ap_server_conf, APLOGNO(00294) "Accept mutex: %s (default: %s)",
                 (all_buckets[0].mutex)
    ...
    {
        ap_log_error(APLOG_MARK, APLOG_NOTICE, 0, ap_server_conf, APLOGNO(00297) AP_SIG_GRACEFUL_STRING " received.  Doing graceful restart");
                             ...
                             apr_pool_t *ptemp)
{
    ...
    if (retained->mpm->module_loads == 2)
    {
        if (!one_process && !foreground)
        {
            ...
            if (rv != APR_SUCCESS)
            {
                ap_log_error(APLOG_MARK, APLOG_CRIT, rv, NULL, APLOGNO(00299) "apr_proc_detach failed");
                               ...
                               apr_pool_t *ptemp, server_rec *s)
{
    ...
    if (server_limit > MAX_SERVER_LIMIT)
    {
        if (startup)
        {
            ap_log_error(APLOG_MARK, APLOG_WARNING | APLOG_STARTUP, 0, NULL, APLOGNO(00300) "WARNING: ServerLimit of %d exceeds compile-time "
                                                                                            "limit of %d servers, decreasing to %d.",
                         server_limit, MAX_SERVER_LIMIT, MAX_SERVER_LIMIT);
        }
        else
        {
            ap_log_error(APLOG_MARK, APLOG_WARNING, 0, s, APLOGNO(00301) "ServerLimit of %d exceeds compile-time limit "
    ...
    else if (server_limit < 1)
    {
        if (startup)
        {
            ap_log_error(APLOG_MARK, APLOG_WARNING | APLOG_STARTUP, 0, NULL, APLOGNO(00302) "WARNING: ServerLimit of %d not allowed, "
                                                                                            "increasing to 1.",
                         server_limit);
        }
        else
    ...
    else if (server_limit != retained->first_server_limit)
    {
        ...
        ap_log_error(APLOG_MARK, APLOG_WARNING, 0, s, APLOGNO(00304) "changing ServerLimit to %d from original value of %d "
    ...
    if (thread_limit > MAX_THREAD_LIMIT)
    {
        if (startup)
        {
            ap_log_error(APLOG_MARK, APLOG_WARNING | APLOG_STARTUP, 0, NULL, APLOGNO(00305) "WARNING: ThreadLimit of %d exceeds compile-time "
                                                                                            "limit of %d threads, decreasing to %d.",
                         thread_limit, MAX_THREAD_LIMIT, MAX_THREAD_LIMIT);
        }
        else
        {
            ap_log_error(APLOG_MARK, APLOG_WARNING, 0, s, APLOGNO(00306) "ThreadLimit of %d exceeds compile-time limit "
                                                                         "of %d, decreasing to match",
    ...
    else if (thread_limit < 1)
    {
        if (startup)
        {
            ap_log_error(APLOG_MARK, APLOG_WARNING | APLOG_STARTUP, 0, NULL, APLOGNO(00307) "WARNING: ThreadLimit of %d not allowed, "
                                                                                            "increasing to 1.",
                         thread_limit);
        }
        else
    ...
    {
        /* don't need a startup console version here */
        ap_log_error(APLOG_MARK, APLOG_WARNING, 0, s, APLOGNO(00309) "changing ThreadLimit to %d from original value of %d "
                                                                     "not allowed during restart",
    ...
    if (threads_per_child > thread_limit)
    {
        if (startup)
        {
            ap_log_error(APLOG_MARK, APLOG_WARNING | APLOG_STARTUP, 0, NULL, APLOGNO(00310) "WARNING: ThreadsPerChild of %d exceeds ThreadLimit "
        ...
        {
            ap_log_error(APLOG_MARK, APLOG_WARNING, 0, s, APLOGNO(00311) "ThreadsPerChild of %d exceeds ThreadLimit "
                                                                         "of %d, decreasing to match",
    ...
    else if (threads_per_child < 1)
    {
        if (startup)
        {
            ap_log_error(APLOG_MARK, APLOG_WARNING | APLOG_STARTUP, 0, NULL, APLOGNO(00312) "WARNING: ThreadsPerChild of %d not allowed, "
                                                                                            "increasing to 1.",
                         threads_per_child);
        }
        else
    ...
    if (max_workers < threads_per_child)
    {
        if (startup)
        {
            ap_log_error(APLOG_MARK, APLOG_WARNING | APLOG_STARTUP, 0, NULL, APLOGNO(00314) "WARNING: MaxRequestWorkers of %d is less than "
        ...
        {
            ap_log_error(APLOG_MARK, APLOG_WARNING, 0, s, APLOGNO(00315) "MaxRequestWorkers of %d is less than ThreadsPerChild "
    ...
    if (max_workers % threads_per_child)
    {
        ...
        if (startup)
        {
            ap_log_error(APLOG_MARK, APLOG_WARNING | APLOG_STARTUP, 0, NULL, APLOGNO(00316) "WARNING: MaxRequestWorkers of %d is not an integer "
        ...
        {
            ap_log_error(APLOG_MARK, APLOG_WARNING, 0, s, APLOGNO(00317) "MaxRequestWorkers of %d is not an integer multiple of "
    ...
    if (ap_daemons_limit > server_limit)
    {
        if (startup)
        {
            ap_log_error(APLOG_MARK, APLOG_WARNING | APLOG_STARTUP, 0, NULL, APLOGNO(00318) "WARNING: MaxRequestWorkers of %d would require %d "
        ...
        {
            ap_log_error(APLOG_MARK, APLOG_WARNING, 0, s, APLOGNO(00319) "MaxRequestWorkers of %d would require %d servers and "
    ...
    if (ap_daemons_to_start < 1)
    {
        if (startup)
        {
            ap_log_error(APLOG_MARK, APLOG_WARNING | APLOG_STARTUP, 0, NULL, APLOGNO(00320) "WARNING: StartServers of %d not allowed, "
                                                                                            "increasing to 1.",
                         ap_daemons_to_start);
        }
        else
    ...
    if (min_spare_threads < 1)
    {
        if (startup)
        {
            ap_log_error(APLOG_MARK, APLOG_WARNING | APLOG_STARTUP, 0, NULL, APLOGNO(00322) "WARNING: MinSpareThreads of %d not allowed, "
                                   ...
                                   const char *arg)
{
    ...
    {
        ap_log_error(APLOG_MARK, APLOG_INFO, 0, NULL, APLOGNO(00324) "MaxClients is deprecated, use MaxRequestWorkers "
                                                                     "instead.");
    }
