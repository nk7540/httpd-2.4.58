                                 ...
                                 server_rec *virt)
{
    ...
    for (modp = ap_top_module; modp; modp = modp->next)
    {
        ...
        if (!virt_vector[i])
        {
            if (df && modp->create_server_config && (ap_get_module_flags(modp) & AP_MODULE_FLAG_ALWAYS_MERGE))
            {
...
AP_CORE_DECLARE(ap_conf_vector_t *)
ap_create_request_config(apr_pool_t *p)
{
...
AP_CORE_DECLARE(ap_conf_vector_t *)
ap_create_per_dir_config(apr_pool_t *p)
{
    return create_empty_config(p);
}
...
ap_invoke_handler(request_rec *r)
{
    ...
    if ((result != OK) && (result != DONE) && (result != DECLINED) && (result != SUSPENDED) && (result != AP_FILTER_ERROR) /* ap_die() knows about this specifically */
...
AP_DECLARE(int)
ap_method_is_limited(cmd_parms *cmd, const char *method)
{
    int methnum;
...
AP_DECLARE(void)
ap_register_hooks(module *m, apr_pool_t *p)
{
    if (m->register_hooks)
    {
...
ap_add_module(module *m, apr_pool_t *p,
              const char *sym_name)
{
...
AP_DECLARE(const char *)
ap_add_loaded_module(module *mod, apr_pool_t *p,
...
AP_DECLARE(const char *)
ap_setup_prelinked_modules(process_rec *process)
{
    module **m;
    module **m2;
    const char *error;
...
AP_DECLARE(const char *)
ap_find_module_short_name(int module_index)
{
...
ap_set_config_vectors(server_rec *server,
...
AP_DECLARE(const char *)
ap_build_cont_config(apr_pool_t *p,
                     ...
                     char *orig_directive)
{
    ...
    while ((rc = ap_varbuf_cfg_getline(&vb, parms->config_file, max_len)) == APR_SUCCESS)
    {
        if (!memcmp(vb.buf, "</", 2) && (ap_cstr_casecmp(vb.buf + 2, bracket) == 0) && (*curr_parent == NULL))
        {
            break;
        }
...
AP_DECLARE(const char *)
ap_walk_config(ap_directive_t *current,
               cmd_parms *parms,
               ap_conf_vector_t *section_vector)
{
...
ap_set_string_slot(cmd_parms *cmd,
                   void *struct_ptr,
                   const char *arg)
{
...
AP_DECLARE_NONSTD(const char *)
ap_set_string_slot_lower(cmd_parms *cmd,
                         void *struct_ptr,
                         const char *arg_)
{
...
AP_DECLARE_NONSTD(const char *)
ap_set_flag_slot(cmd_parms *cmd,
...
AP_DECLARE_NONSTD(const char *)
ap_set_flag_slot_char(cmd_parms *cmd,
                      void *struct_ptr_v, int arg)
{
    int offset = (int)(long)cmd->info;
    char *struct_ptr = (char *)struct_ptr_v;
...
AP_DECLARE_NONSTD(const char *)
ap_set_deprecated(cmd_parms *cmd,
                  void *struct_ptr,
                  const char *arg)
{
...
AP_DECLARE(void)
ap_reset_module_loglevels(struct ap_logconf *l, int val)
{
    if (l->module_levels)
...
AP_DECLARE(void)
ap_set_module_loglevel(apr_pool_t *pool, struct ap_logconf *l,
                       int index, int level)
{
    if (!l->module_levels)
    {
...
AP_DECLARE(char *)
ap_server_root_relative(apr_pool_t *p, const char *file)
{
    char *newpath = NULL;
    apr_status_t rv;
    rv = apr_filepath_merge(&newpath, ap_server_root, file,
                            APR_FILEPATH_TRUENAME, p);
    if (newpath && (rv == APR_SUCCESS || APR_STATUS_IS_EPATHWILD(rv) || APR_STATUS_IS_ENOENT(rv) || APR_STATUS_IS_ENOTDIR(rv)))
    {
        return newpath;
    }
    else
...
AP_DECLARE(char *)
ap_runtime_dir_relative(apr_pool_t *p, const char *file)
{
    char *newpath = NULL;
    apr_status_t rv;
...
AP_DECLARE(const char *)
ap_soak_end_container(cmd_parms *cmd, char *directive)
{
    struct ap_varbuf vb;
    const char *args;
    char *cmd_name;
    ...
    while ((rc = ap_varbuf_cfg_getline(&vb, cmd->config_file, max_len)) == APR_SUCCESS)
    {
        args = vb.buf;
...
AP_DECLARE(const char *)
ap_process_resource_config(server_rec *s,
                           const char *fname,
                           ap_directive_t **conftree,
                           apr_pool_t *p,
                           apr_pool_t *ptemp)
...
AP_DECLARE(const char *)
ap_process_fnmatch_configs(server_rec *s,
                           const char *fname,
                           ap_directive_t **conftree,
                           apr_pool_t *p,
                           apr_pool_t *ptemp,
                           int optional)
{
    ...
    if ((ap_server_pre_read_config->nelts || ap_server_post_read_config->nelts) && !(strcmp(fname, ap_server_root_relative(ptemp, SERVER_CONFIG_FILE))))
    {
        apr_finfo_t finfo;
...
AP_DECLARE(int)
ap_process_config_tree(server_rec *s,
                       ap_directive_t *conftree,
                       apr_pool_t *p,
                       apr_pool_t *ptemp)
{
    ...
    {
        if (parms.err_directive)
            ap_log_perror(APLOG_MARK, APLOG_STARTUP, 0, p, APLOGNO(00526) "Syntax error on line %d of %s:",
                          parms.err_directive->line_num,
...
AP_CORE_DECLARE(int)
ap_parse_htaccess(ap_conf_vector_t **result,
                  ...
                  const char *d, const char *access_names)
{
    ...
    while (access_names[0])
    {
        ...
        else
        {
            ...
            {
                ap_log_rerror(APLOG_MARK, APLOG_CRIT, status, r, APLOGNO(00529) "%s pcfg_openfile: unable to check htaccess file, "
...
ap_init_virtual_host(apr_pool_t *p,
                     const char *hostname,
...
AP_DECLARE(struct ap_logconf *)
ap_new_log_config(apr_pool_t *p,
...
AP_DECLARE(void)
ap_merge_log_config(const struct ap_logconf *old_conf,
...
AP_DECLARE(void)
ap_fixup_virtual_hosts(apr_pool_t *p, server_rec *main_server)
{
    server_rec *virt;
...
AP_CORE_DECLARE(void)
ap_register_config_hooks(apr_pool_t *p)
{
    ap_hook_pre_config(conf_vector_length_pre_config, NULL, NULL,
                       APR_HOOK_REALLY_LAST);
}

AP_DECLARE(server_rec *)
ap_read_config(process_rec *process, apr_pool_t *ptemp,
               const char *filename,
               ap_directive_t **conftree)
{
    ...
    if (error)
    {
        ap_log_error(APLOG_MARK, APLOG_STARTUP | APLOG_CRIT, 0, NULL, APLOGNO(00534) "%s: Configuration error: %s", ap_server_argv0, error);
        return NULL;
    }
...
AP_DECLARE(void)
ap_single_module_configure(apr_pool_t *p, server_rec *s,
                           module *m)
{
...
AP_DECLARE(void)
ap_run_rewrite_args(process_rec *process)
{
    module *m;
...
static void show_overrides(const command_rec *pc, module *pm)
{
    ...
    if ((pc->req_override & (OR_OPTIONS | OR_FILEINFO | OR_INDEXES)) || ((pc->req_override & RSRC_CONF) && ((pc->req_override & (ACCESS_CONF | OR_AUTHCFG | OR_LIMIT)))))
    {
        printf("anywhere");
    }
...
AP_DECLARE(int)
ap_exists_directive(apr_pool_t *p, const char *name)
{
...
AP_DECLARE(void *)
ap_retained_data_get(const char *key)
{
    void *retained;
...
AP_DECLARE(void *)
ap_retained_data_create(const char *key, apr_size_t size)
{
    void *retained;
...
AP_DECLARE(void)
ap_reserve_module_slots(int count)
{
    reserved_module_slots += count;
}

AP_DECLARE(void)
ap_reserve_module_slots_directive(const char *directive)
{
