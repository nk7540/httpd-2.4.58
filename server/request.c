...
static int decl_die(int status, const char *phase, request_rec *r)
{
    if (status == DECLINED)
    {
        ap_log_rerror(APLOG_MARK, APLOG_CRIT, 0, r, APLOGNO(00025) "configuration error:  couldn't %s: %s", phase, r->uri);
...
AP_DECLARE(int)
ap_some_authn_required(request_rec *r)
{
    int access_status;
    char *olduser = r->user;
    int rv = FALSE;
...
ap_process_request_internal(request_rec *r)
{
    ...
    if (r->parsed_uri.path)
    {
        ...
        {
            ap_log_rerror(APLOG_MARK, APLOG_ERR, 0, r, APLOGNO(10244) "invalid URI path (%s)", r->unparsed_uri);
    ...
    if (access_status != DONE && r->parsed_uri.path)
    {
        ...
        if (access_status)
        {
            if (access_status == HTTP_NOT_FOUND)
            {
                if (!d->allow_encoded_slashes)
                {
                    ap_log_rerror(APLOG_MARK, APLOG_INFO, 0, r, APLOGNO(00026) "found %%2f (encoded '/') in URI path (%s), "
                                                                               "returning 404",
                                  r->unparsed_uri);
                }
            }
    ...
    else
    {
        ...
        */
        ...
        case SATISFY_NOSPEC:
            ...
            access_status = ap_run_access_checker_ex(r);
            if (access_status == DECLINED || (access_status == OK && ap_run_force_authn(r) == OK))
            {
                ...
                if (r->user == NULL)
                {
                    ...
                    ap_log_rerror(APLOG_MARK, APLOG_ERR, 0, r, APLOGNO(00027) "No authentication done but request not "
        ...
        case SATISFY_ANY:
            ...
            if (access_status == DECLINED || (access_status == OK && ap_run_force_authn(r) == OK))
            {
                ...
                {
                    /* don't let buggy authn module crash us in authz */
                    ap_log_rerror(APLOG_MARK, APLOG_ERR, 0, r, APLOGNO(00028) "No authentication done but request not "
                                                                              "allowed without authentication for %s. "
                                                                              "Authentication not configured?",
                                  r->uri);
                    access_status = HTTP_INTERNAL_SERVER_ERROR;
...
static walk_cache_t *prep_walk_cache(apr_size_t t, request_rec *r)
{
    ...
    if ((r->prev && (inherit_note = ap_get_request_note(r->prev, t)) && *inherit_note) || (r->main && (inherit_note = ap_get_request_note(r->main, t)) && *inherit_note))
    {
...
static int resolve_symlink(char *d, apr_finfo_t *lfi, int opts, apr_pool_t *p)
{
    ...
    {
        if (apr_stat(&fi, d, lfi->valid & ~(APR_FINFO_NAME | APR_FINFO_LINK), p) != APR_SUCCESS)
        {
            return HTTP_FORBIDDEN;
        }
    ...
    {
        if (apr_stat(lfi, d, lfi->valid | APR_FINFO_LINK | APR_FINFO_OWNER, p) != APR_SUCCESS)
        {
            return HTTP_FORBIDDEN;
        }
    }
...
static void core_opts_merge(const ap_conf_vector_t *sec, core_opts_t *opts)
{
    ...
    {
        opts->add = (opts->add & ~this_dir->opts_remove) | this_dir->opts_add;
        opts->remove = (opts->remove & ~this_dir->opts_add) | this_dir->opts_remove;
...
AP_DECLARE(int)
ap_directory_walk(request_rec *r)
{
    ...
    if ((rv = apr_filepath_merge(&entry_dir, NULL, r->filename,
                                 APR_FILEPATH_NOTRELATIVE, r->pool)) != APR_SUCCESS)
    {
        ap_log_rerror(APLOG_MARK, APLOG_INFO, 0, r, APLOGNO(00030) "Module bug?  Request filename path %s is invalid or "
     ...
     */
    if (cached && ((r->finfo.filetype == APR_REG) || ((r->finfo.filetype == APR_DIR) && (!r->path_info || !*r->path_info))) && (cache->dir_conf_tested == sec_ent) && (strcmp(entry_dir, cache->cached) == 0))
    {
        int familiar = 0;
            ...
            if (!(opts & OPT_SYM_LINKS))
            {
                ...
                if (thisinfo.filetype == APR_LNK)
                {
                    ...
                    if ((res = resolve_symlink(r->filename, &thisinfo,
                                               opts, r->pool)) != OK)
                    {
                        ap_log_rerror(APLOG_MARK, APLOG_ERR, 0, r, APLOGNO(00032) "Symbolic link not allowed "
    ...
    else
    {
        ...
        if ((r->finfo.filetype == APR_DIR) && r->path_info && *r->path_info)
        {
            if ((rv = apr_filepath_merge(&r->path_info, r->filename,
                                         r->path_info,
                                         APR_FILEPATH_NOTABOVEROOT, r->pool)) != APR_SUCCESS)
            {
                ap_log_rerror(APLOG_MARK, APLOG_ERR, rv, r, APLOGNO(00033) "dir_walk error, path_info %s is not relative "
        ...
        while (canonical_len && ((r->canonical_filename[canonical_len - 1] != '/' && r->canonical_filename[canonical_len - 1]) || (r->path_info[canonical_len - 1] != '/' && r->path_info[canonical_len - 1])))
        {
            --canonical_len;
        }
        ...
        if (rv != APR_SUCCESS)
        {
            ap_log_rerror(APLOG_MARK, APLOG_ERR, rv, r, APLOGNO(00034) "dir_walk error, could not determine the root "
        ...
        do
        {
            ...
            for (; sec_idx < num_sec; ++sec_idx)
            {
                 ...
                 */
                if (entry_core->d_components && ((entry_core->d_components < seg) || (entry_core->d_is_fnmatch
            ...
            { /* Not really a loop, just a break'able code block */
                 ...
                 */
                if (seg < startseg || (!opts.override && apr_is_empty_table(opts.override_list)))
                {
                    break;
                }
            ...
            {
                ap_log_rerror(APLOG_MARK, APLOG_ERR, rv, r, APLOGNO(00035) "access to %s denied (filesystem path '%s') "
                                                                           "because search permissions are missing on a "
                                                                           "component of the path",
                              r->uri, r->filename);
                return r->status = HTTP_FORBIDDEN;
            }
            else if ((rv != APR_SUCCESS && rv != APR_INCOMPLETE) || !(thisinfo.valid & APR_FINFO_TYPE))
            {
             ...
             */
            if ((thisinfo.valid & APR_FINFO_NAME) && strcmp(seg_name, thisinfo.name))
            {
            ...
            if (thisinfo.filetype == APR_LNK)
            {
                ...
                if ((res = resolve_symlink(r->filename, &thisinfo,
                                           opts.opts, r->pool)) != OK)
                {
                    ap_log_rerror(APLOG_MARK, APLOG_ERR, 0, r, APLOGNO(00037) "Symbolic link not allowed "
                                                                              "or link target not accessible: %s",
                                  r->filename);
                    return r->status = res;
                }
            ...
            {
                ap_log_rerror(APLOG_MARK, APLOG_ERR, 0, r, APLOGNO(00038) "Forbidden: %s doesn't point to "
                                                                          "a file or directory",
                              r->filename);
                return r->status = HTTP_FORBIDDEN;
            }
...
ap_location_walk(request_rec *r)
{
     ...
     */
    if (cached && (cache->dir_conf_tested == sec_ent) && (strcmp(entry_uri, cache->cached) == 0))
    {
        /* Well this looks really familiar!  If our end-result (per_dir_result)
         * didn't change, we have absolutely nothing to do :)
         * Otherwise (as is the case with most dir_merged/file_merged requests)
        ...
        for (sec_idx = 0; sec_idx < num_sec; ++sec_idx)
        {
            ...
            else
            {
                ...
                if ((entry_core->d_is_fnmatch
                         ...
                         : (strncmp(entry_core->d, cache->cached, len) || (len > 0 && entry_core->d[len - 1] != '/' && cache->cached[len] != '/' && cache->cached[len] != '\0'))))
                {
                    continue;
                }
            }
...
ap_file_walk(request_rec *r)
{
     ...
     */
    if (cached && (cache->dir_conf_tested == sec_ent) && (strcmp(test_file, cache->cached) == 0))
    {
        /* Well this looks really familiar!  If our end-result (per_dir_result)
         * didn't change, we have absolutely nothing to do :)
...
static int ap_if_walk_sub(request_rec *r, core_dir_config *dconf)
{
    ...
    for (sec_idx = 0; sec_idx < num_sec; ++sec_idx)
    {
        ...
        if (entry_core->condition_ifelse & AP_CONDITION_IF)
        {
            ...
            {
                if (rc < 0)
                    ap_log_rerror(APLOG_MARK, APLOG_ERR, 0, r, APLOGNO(00039) "Failed to evaluate <If > condition: %s",
                                  err);
                prev_result = 0;
                continue;
            }
...
ap_sub_req_output_filter(ap_filter_t *f,
...
AP_DECLARE(void)
ap_setup_auth_internal(apr_pool_t *ptemp)
{
...
ap_register_auth_provider(apr_pool_t *pool,
...
AP_DECLARE(void)
ap_hook_check_access(ap_HOOK_access_checker_t *pf,
...
AP_DECLARE(void)
ap_hook_check_access_ex(ap_HOOK_access_checker_ex_t *pf,
                        const char *const *aszPre,
                        const char *const *aszSucc,
                        int nOrder, int type)
{
...
AP_DECLARE(void)
ap_hook_check_authn(ap_HOOK_check_user_id_t *pf,
                    const char *const *aszPre,
                    const char *const *aszSucc,
                    int nOrder, int type)
{
...
AP_DECLARE(void)
ap_hook_check_authz(ap_HOOK_auth_checker_t *pf,
                    const char *const *aszPre,
                    const char *const *aszSucc,
                    int nOrder, int type)
{
...
AP_DECLARE(request_rec *)
ap_sub_req_method_uri(const char *method,
                      const char *new_uri,
                      const request_rec *r,
                      ap_filter_t *next_filter)
{
...
AP_DECLARE(request_rec *)
ap_sub_req_lookup_uri(const char *new_uri,
                      const request_rec *r,
                      ap_filter_t *next_filter)
{
...
AP_DECLARE(request_rec *)
ap_sub_req_lookup_dirent(const apr_finfo_t *dirent,
                         const request_rec *r,
                         int subtype,
                         ap_filter_t *next_filter)
{
...
AP_DECLARE(request_rec *)
ap_sub_req_lookup_file(const char *new_file,
                       const request_rec *r,
                       ap_filter_t *next_filter)
{
    request_rec *rnew;
 ...
 */
AP_DECLARE(void)
ap_update_mtime(request_rec *r, apr_time_t dependency_mtime)
{
