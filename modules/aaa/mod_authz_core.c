                                                    ...
                                                    const void *parsed_require_args)
{
  ...
  ap_log_rerror(APLOG_MARK, APLOG_ERR, 0, r, APLOGNO(02305) "no alias provider found for '%s' (BUG?)",
                                               ...
                                               const char *args)
{
  ...
  {
    ap_log_error(APLOG_MARK, APLOG_WARNING, 0, cmd->server, APLOGNO(10142) "When several arguments (%s %s...) are passed to a %s directive, "
                                    ...
                                    authz_section_conf *section, int is_conf)
{
  ...
  if (ret != OK)
  {
    ap_log_error(APLOG_MARK, APLOG_ERR | APLOG_STARTUP, APR_SUCCESS, s, APLOGNO(01624) "%s directive contains only negative authorization directives",
                                         ...
                                         authz_logic_op parent_op)
{
  ...
  if (!(section->limited & (AP_METHOD_BIT << r->method_number)))
  {
    ...
    ap_log_rerror(APLOG_MARK, APLOG_DEBUG, APR_SUCCESS, r, APLOGNO(01625) "authorization result of %s: %s "
    ...
    while (child)
    {
      ...
      if (child_result != AUTHZ_NEUTRAL)
      {
        ...
        if (section->op == AUTHZ_LOGIC_AND)
        {
          ...
          if ((child_result == AUTHZ_DENIED_NO_USER && auth_result != AUTHZ_DENIED) || (auth_result == AUTHZ_NEUTRAL))
          {
            auth_result = child_result;
          }
        }
...
static int authorize_user_core(request_rec *r, int after_authn)
{
  ...
  if (!conf->section)
  {
    if (ap_auth_type(r))
    {
      ...
      ap_log_rerror(APLOG_MARK, APLOG_ERR, APR_SUCCESS, r, APLOGNO(01627) "AuthType configured with no corresponding "
                                                                          "authorization directives");

      return HTTP_INTERNAL_SERVER_ERROR;
    }

    ap_log_rerror(APLOG_MARK, APLOG_DEBUG, APR_SUCCESS, r, APLOGNO(01628) "authorization result: granted (no directives)");
  ...
  else if (auth_result == AUTHZ_DENIED || auth_result == AUTHZ_NEUTRAL)
  {
    ...
    {
      ap_log_rerror(APLOG_MARK, APLOG_ERR, APR_SUCCESS, r, APLOGNO(01631) "user %s: authorization failure for \"%s\": ",
                    r->user, r->uri);
...
static int authz_some_auth_required(request_rec *r)
{
  ...
  if (conf->section && (conf->section->limited & (AP_METHOD_BIT << r->method_number)))
  {
    return 1;
  }
...
{
  if (parms->type == AP_EXPR_FUNC_VAR && strcasecmp(parms->name, "REMOTE_USER") == 0)
  {
                                             ...
                                             const void *parsed_require_line)
{
  ...
  if (rc < 0)
  {
    ap_log_rerror(APLOG_MARK, APLOG_ERR, 0, r, APLOGNO(02320) "Error evaluating expression in 'Require expr': %s",
                  err);
    return AUTHZ_GENERAL_ERROR;
  }
  else if (rc == 0)
